...
- `LT1/Spc` = 탭하면 Space, 홀드하면 number 레이어. 세벌식 390에선 숫자를 레이어로 자주 꺼내므로, 손을 떼지 않고 스페이스 홀드만으로 number 레이어에 바로 접근하려는 배치
...
- 세벌식 390은 기본 레이어의 숫자열(`1~0`)을 한글 자모에 쓰므로, 이 레이어가 숫자를 되찾아주는 역할 → 자연스럽게 연결됨
- 전환키로 **접근성 좋은 좌측 스페이스(`LT1`)와 Fn 자리(`MO1`, HHKB Fn 위치인 우하단)** 를 둘 다 골라, 손버릇대로 빠르게 숫자에 진입
- 상단은 숫자열, 오른손(`H J K L` / `N M , .`)이 넘패드 `4 5 6` / `0 1 2 3` — 세벌식 390이 숫자를 넘패드식으로 입력하던 감각을 그대로 옮긴 것
...
- 기호를 **오른쪽 열에 몰아서** 배치 — 일반 키보드에서 특수문자를 우측 열부터 오른손으로 집던 손 감각을 그대로 유지
...
- specialA와 같은 원칙 — `- = \ \`` 기호를 우측 열에 배치해 오른손 접근 감각 유지
...
- 오른손 홈 근처에 방향키(`↑ ← ↓ →`) 배치 — HHKB Fn 레이어에서 방향키를 오른손으로 집던 손버릇을 그대로 옮긴 구성
- `Sleep`(Z 자리, **z**zz) = mac 화면 슬립(Ctrl+Shift+Power), `Boot`(B 자리, **B**oot) = 부트로더 진입 — 니모닉으로 배치
